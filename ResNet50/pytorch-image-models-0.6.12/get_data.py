...
def balance_per_class(args, df):
    ...
    for i in range(1000):
        ...
        result = pd.concat([result, df_nature_id.sample(n=n_to_sample, random_state=42),
...
def get_data(args, is_training=True, balance_val_classes=False, balance_train_classes=False):
    ...
    if is_training:
        if args.dataset.lower() == "CLASSIC".lower():
            ...
            data_train_real = df[(df["mode"] == "train") & (df["generator"] == "nature") & \
                                 (df["path"].apply(lambda x: x.split("/")[-4]) == args.generator) & (
                                             df["compression_rate"] == 96)].sample(n=37000)
            ...
            if args.wildrf:
                ...
                if not balance_train_classes:
                    data_train_ai = df[(df["mode"] == "train")].sample(
                        n=len(data_train_real), random_state=42)
                else:
                    data_train_ai = df[(df["mode"] == "train")]
                ...
                data_val_real = df[(df["mode"] == "val") & (df["generator"] == "nature") & \
                                   (df["width"] >= args.min_size) & (df["height"] >= args.min_size) & (
                                           ...
                                           df["compression_rate"] == args.jpeg_qf)]
                if not balance_val_classes:
                    data_val_ai = df[(df["mode"] == "val")].sample(
                        n=len(data_val_real), random_state=42)
                else:
                    data_val_ai = df[(df["mode"] == "val")]
                data_val = pd.concat([data_val_real, data_val_ai])
        ...
        if args.dataset.lower() == "FFHQ_JPEG".lower():
            ...
            data_val = df[(df["generator"] == "nature") & ((df["mode"] == "val") | (
                        df["path"].apply(lambda x: x.split("/")[-4]) != args.generator_trained_on))]
            data_val = data_val[(data_val["width"] >= args.min_width) & (data_val["height"] >= args.min_height) & (
                        data_val["width"] <= args.max_width) & (data_val["height"] <= args.max_height)]
        ...
        #hinzugefügt
        elif args.dataset == "wildrf":
            if args.platform is None:
                data_val = df[(df["path"].apply(lambda x: x.split("/")[-3]) == args.platform)]
