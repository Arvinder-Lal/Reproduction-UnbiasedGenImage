...
_logger = logging.getLogger('get_data')
...
def balance_per_class(args, df):
    ...
    for i in range(1000):
        ...
        result = pd.concat([
            result,
            df_nature_id.sample(n=n_to_sample, random_state=42),
            df_ai_id.sample(n=n_to_sample, random_state=42)
        ])

    return result

#Changed number from -5 to -4 in x.split
    ...
    try:
        ...
        if is_training:
            if args.dataset.lower() == "CLASSIC".lower():
                data_train = df[(df['mode'] == 'train') & (df["path"].apply(lambda x: x.split("/")[-4]) == args.generator)]
                data_val = df[(df['mode'] == 'val') & (df["path"].apply(lambda x: x.split("/")[-4]) == args.generator)]

            elif args.dataset.lower() == "JPEG96".lower():
                data_train_real = df[(df["mode"] == "train") & (df["generator"] == "nature") &
                                    (df["path"].apply(lambda x: x.split("/")[-4]) == args.generator) &
                                    (df["compression_rate"] == 96)]

                data_train_ai = df[(df["mode"] == "train") & (df["generator"] == args.generator)]
                data_train_ai = data_train_ai.sample(n=len(data_train_real)) if len(data_train_real) > 0 else data_train_ai
                ...
                data_val_ai = data_val_ai.sample(n=len(data_val_real)) if len(data_val_real) > 0 else data_val_ai
                ...
                data_train_real = df[(df["mode"] == "train") & (df["generator"] == "nature") &
                                    (df["width"] >= args.min_size) & (df["height"] >= args.min_size) &
                                    (df["width"] <= args.max_size) & (df["height"] <= args.max_size) &
                                    (df["compression_rate"] == args.jpeg_qf)]

                if not balance_train_classes:
                    data_train_ai = df[(df["mode"] == "train") & (df["generator"] == args.generator)].sample(
                        n=len(data_train_real), random_state=42)
                else:
                    data_train_ai = df[(df["mode"] == "train") & (df["generator"] == args.generator)]
                ...
                if not balance_val_classes:
                    data_val_ai = df[(df["mode"] == "val") & (df["generator"] == args.generator)].sample(
                        n=len(data_val_real), random_state=42)
                else:
                    data_val_ai = df[(df["mode"] == "val") & (df["generator"] == args.generator)]
                data_val = pd.concat([data_val_real, data_val_ai])
                ...
                raise NotImplementedError(
                    "If training, args.dataset must be one of classic, jpeg96 or size_constrained")
            ...
            if args.dataset.lower() == "FFHQ_JPEG".lower():
                ...
                data_val = df[(df["generator"] == "nature") &
                            ((df["mode"] == "val") | (df["path"].apply(lambda x: x.split("/")[-4]) != args.generator_trained_on))]
                data_val = data_val[(data_val["width"] >= args.min_width) &
                                  (data_val["height"] >= args.min_height) &
                                  (data_val["width"] <= args.max_width) &
                                  (data_val["height"] <= args.max_height)]
                ...
                data_val = df[(df["path"].apply(lambda x: x.split("/")[-4]) == args.generator) &
                            (df["mode"] == "val") &
                            (df["compression_rate"] >= args.jpeg_qf)]
                ...
                raise NotImplementedError(
                    "If not training (testing/validating), args.dataset must be one of ffhq_jpeg, imagenet_jpeg, size, classic. See ReadMe markdown file ")
